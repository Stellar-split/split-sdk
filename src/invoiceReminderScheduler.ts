...
export class InvoiceReminderScheduler extends TypedEventEmitter<InvoiceReminderSchedulerEventMap> {
  ...
  /**
   * Schedule a single reminder for an invoice and return its opaque reminder ID.
   * Persists the schedule immediately so it survives a restart.
   */
  ...
  /**
   * Cancel a single reminder by its opaque reminder ID.
   * Returns `true` when the reminder was pending and is now cancelled;
  ...
  /**
   * Return all not-yet-fired, not-cancelled reminders as lightweight
   * `{ reminderId, invoiceId, remindAt }` descriptors.
   */
  getPendingReminders(): Array<{ reminderId: string; invoiceId: string; remindAt: number }> {
    return this.schedules
      .filter((s) => s.status === "pending")
      .map((s) => ({ reminderId: s.id, invoiceId: s.invoiceId, remindAt: s.fireAt }));
  }

  /** Clear all reminders and their timers. Useful for testing/teardown. */
